...
po_aov <- aov(outcome ~ treatment, data = po1)
summary(po_aov)
cohen.d(po$Y_D_0, po$Y_D_1, paired = TRUE)

samp <- sam %>%
  select(villages, Y_D_0, Y_D_1) %>% 
  pivot_longer(cols = starts_with("Y_D"), names_to = "treatment",values_to="outcome")

sam_aov <- aov(outcome ~ treatment, data = samp)
summary(sam_aov)
cohen.d(sam$Y_D_0, sam$Y_D_1, paired = TRUE)

samp2 <- sam2 %>%
  select(villages, Y_D_0, Y_D_1) %>% 
  pivot_longer(cols = starts_with("Y_D"), names_to = "treatment",values_to="outcome")

sam2_aov <- aov(outcome ~ treatment, data = samp2)
summary(sam2_aov)
cohen.d(sam2$Y_D_0, sam2$Y_D_1)
```
